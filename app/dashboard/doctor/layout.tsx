...
export default function DoctorLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setSearching(true);
    try {
      const data = await apiRequest(`/patient?search=${encodeURIComponent(searchQuery)}`);
        ...
        <aside className="fixed left-0 top-16 bottom-0 w-64 bg-white border-r border-gray-200 flex flex-col">
              ...
              <div className="mt-2 border border-gray-100 rounded-lg overflow-hidden">
                {searchResults.map((p) => (
                  <button
                    ...
                    className="w-full text-left text-sm px-3 py-2 hover:bg-gray-50 text-gray-700 border-b border-gray-50 last:border-0"
                  >
                    {p.user.name}
                  </button>
                ))}
          ...
          <div className="p-3 border-t border-gray-100">
            <button
              ...
              className="w-full text-left px-3 py-2.5 rounded-lg text-sm text-red-600 hover:bg-red-50 flex items-center gap-3"
            >
